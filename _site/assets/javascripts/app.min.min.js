...
angular.module('app.states', ['app.components', 'ui.router']).config(['$stateProvider', '$urlRouterProvider', function ($stateProvider, $urlRouterProvider) {
  ...
  $stateProvider.state('feed', {
    ...
    url: '/company/:symbol',
    templateProvider: function templateProvider($stateParams) {
...
angular.module('app.factories', []).factory('apiRealTimeQuotes', ['$http', function apiRealTimeQuotesFactory($http) {
  ...
  return {
    get: function get(symbol, timeSpan) {
      var market = config.NASDAQ[symbol] ? 'NASDAQ' : 'LON',
          collapse = '';
...
angular.module('app', ['app.states', 'app.factories', 'app.components']).controller('ctrl', ['$scope', 'apiRealTimeQuotes', function ($scope, apiRealTimeQuotes) {
...
angular.module('app.components').component('traderAside', {
  templateUrl: 'app/components/aside/view.html',
  controller: function controller() {}
});
'use strict';

angular.module('app.components').component('traderCompany', {
  bindings: {
    user: '=',
    symbol: '@'
  },
  templateUrl: 'app/components/company/view.html',
  controller: function controller(ApiCompanyQuotes) {
    var _this = this;
    ...
    this.getCompany = function () {
      ...
      ApiCompanyQuotes.get(_this.symbol, _this.timeSpan).then(function (response) {
        console.log('here');
        // Making pull data from Quandl available as this.company variable.
        _this.company = response.data.dataset;
      }).then(function () {
        // Plotting the graph.
        plotGraph(_this.company.data);
      }).catch(function (error) {
        console.error(error);
      });
    };
...
angular.module('app.components').component('traderFeed', {
  bindings: {
    user: '=',
    quotes: '='
  },
...
angular.module('app.components').component('traderHeader', {
  bindings: {
    user: '='
  },
  ...
  controller: function controller() {
    ...
    this.login = {};
    this.logIn = function () {
      firebase.auth().signInWithEmailAndPassword(_this.login.email, _this.login.password).then(function () {
        firebase.database().ref('/users/' + firebase.auth().currentUser.uid).once('value').then(function (snapshot) {
        ...
        _this.login.errorMessage = error.message;
        $scope.$apply();
...
angular.module('app.components').component('traderPortfolio', {
  bindings: {
    user: '=',
    quotes: '='
  },
...
angular.module('app.components').component('traderQuotes', {
  bindings: {
    quotes: '='
  },
...
angular.module('app.components').component('traderSettings', {
  bindings: {
    user: '='
  },
...
.component('traderSignup', {
  ...
  controller: function controller($state, $scope) {
    ...
    $scope.$watch('this.user', function () {
      console.info(_this.user);
      console.info(_this.user);
      console.info(_this.user);
    });
    ...
    this.signUp = function () {
      firebase.auth().createUserWithEmailAndPassword(_this.signup.email, _this.signup.password).catch(function (error) {
        _this.signup.errorMessage = error.message;
        // this.$apply();
        ...
        firebase.auth().currentUser.updateProfile({
          displayName: _this.signup.firstName + ' ' + _this.signup.lastName,
          photoURL: 'https://maciejcaputa.com/assets/images/thumbnail-face.jpg'
        }).then(function () {
          firebase.database().ref('users/' + firebase.auth().currentUser.uid).set({
            firstName: _this.signup.firstName,
            lastName: _this.signup.lastName,
            name: _this.signup.firstName + ' ' + _this.signup.lastName,
